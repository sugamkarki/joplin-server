...
describe('shares', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('shares');
    }));
    ...
    test('should retrieve share info', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const itemModel1 = testUtils_1.models().item();
            yield testUtils_1.createItemTree(user1.id, '', tree);
            ...
            const { share } = yield shareApiUtils_1.shareWithUserAndAccept(session1.id, session2.id, user2, types_1.ShareType.Folder, folderItem);
            // Only share with user 3, without accepting it
            yield apiUtils_1.postApi(session1.id, `shares/${share.id}/users`, {
                email: user3.email,
            });
            yield apiUtils_1.postApi(session1.id, 'shares', {
                note_id: noteItem2.jop_id,
            });
            {
                const shares = yield apiUtils_1.getApi(session1.id, 'shares');
                ...
                const shareUsers = yield apiUtils_1.getApi(session1.id, `shares/${share1.id}/users`);
