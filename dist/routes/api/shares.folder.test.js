...
describe('shares.folder', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('shares.folder');
    }));
    ...
    test('should share a folder with another user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const folderItem = yield testUtils_1.createFolder(session1.id, { title: 'created by sharer' });
            // ----------------------------------------------------------------
            // Create the file share object
            // ----------------------------------------------------------------
            const share = yield apiUtils_1.postApi(session1.id, 'shares', {
            ...
            // ----------------------------------------------------------------
            yield testUtils_1.models().item().saveForUser(user1.id, {
                id: folderItem.id,
            ...
            // ----------------------------------------------------------------
            let shareUser = yield apiUtils_1.postApi(session1.id, `shares/${share.id}/users`, {
                email: user2.email,
            });
            shareUser = yield testUtils_1.models().shareUser().load(shareUser.id);
            ...
            // ----------------------------------------------------------------
            yield apiUtils_1.patchApi(session2.id, `share_users/${shareUser.id}`, { status: types_1.ShareUserStatus.Accepted });
            {
                shareUser = yield testUtils_1.models().shareUser().load(shareUser.id);
                expect(shareUser.status).toBe(types_1.ShareUserStatus.Accepted);
            }
            yield testUtils_1.models().share().updateSharedItems3();
            // ----------------------------------------------------------------
            // On the sharee side, check that the file is present
            // and with the right content.
            // ----------------------------------------------------------------
            const results = yield apiUtils_1.getApi(session2.id, 'items/root/children');
            expect(results.items.length).toBe(1);
            expect(results.items[0].name).toBe(folderItem.name);
            const itemContent = yield apiUtils_1.getApi(session2.id, `items/root:/${folderItem.name}:/content`);
            ...
            // ----------------------------------------------------------------
            {
                const folder = yield joplinUtils_1.unserializeJoplinItem(itemContent.toString());
                folder.title = 'modified by recipient';
                yield testUtils_1.updateItem(session2.id, `root:/${folderItem.name}:`, yield joplinUtils_1.serializeJoplinItem(folder));
                // Double check that it didn't create a new item instead of updating it
                expect((yield testUtils_1.models().item().all()).length).toBe(1);
                const modContent = yield apiUtils_1.getApi(session1.id, `items/root:/${folderItem.name}:/content`);
    ...
    test('should share a folder and all its children', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { session: session2 } = yield testUtils_1.createUserAndSession(2);
            yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F3', [
                {
            ...
            const children1 = yield apiUtils_1.getApi(session1.id, 'items/root/children');
            expect(children1.items.length).toBe(8);
            const children2 = yield apiUtils_1.getApi(session2.id, 'items/root/children');
    ...
    test('should received shared items only once invitation accepted', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { shareUser } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
    ...
    test('should share when a note is added to a shared folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { share } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F2', [
                {
                    id: '000000000000000000000000000000F2',
                    children: [
                        {
            ...
            yield testUtils_1.createNote(session1.id, {
                id: '00000000000000000000000000000002',
                parent_id: '000000000000000000000000000000F2',
                share_id: share.id,
            });
            yield testUtils_1.models().share().updateSharedItems3();
            const newChildren = yield testUtils_1.models().item().children(user2.id);
    ...
    test('should update share status when note parent changes', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { share: share1 } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
            ...
            const { share: share2 } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F5', [
                {
                    id: '000000000000000000000000000000F3',
                    children: [],
                },
            ...
            const noteItem = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
    ...
    test('should update share status when note parent changes more than once between updates', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', {
            ...
            const folderItem1 = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F1');
            const noteItem = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
    ...
    test('should unshare a deleted item', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            expect((yield testUtils_1.models().item().children(user2.id)).items.length).toBe(2);
            const noteModel = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            yield testUtils_1.models().item().delete(noteModel.id);
            expect((yield testUtils_1.models().item().children(user2.id)).items.length).toBe(1);
        });
    });
    test('should unshare a deleted shared root folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const folderItem = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F1');
            yield testUtils_1.models().item().delete(folderItem.id);
            ...
            expect((yield testUtils_1.models().item().children(user2.id)).items.length).toBe(0);
            ...
            // one child.
            expect((yield testUtils_1.models().item().children(user1.id)).items.length).toBe(1);
            // Also check that Share and UserShare objects are deleted
            expect((yield testUtils_1.models().share().all()).length).toBe(0);
            expect((yield testUtils_1.models().shareUser().all()).length).toBe(0);
    ...
    test('should unshare when the share object is deleted', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            expect((yield testUtils_1.models().item().children(user2.id)).items.length).toBe(2);
            const share = (yield testUtils_1.models().share().all())[0];
            yield testUtils_1.models().share().delete(share.id);
            expect((yield testUtils_1.models().item().children(user1.id)).items.length).toBe(2);
            expect((yield testUtils_1.models().item().children(user2.id)).items.length).toBe(0);
        });
    });
    ...
    test('should see delta changes for linked items', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let cursor2 = null;
            {
                ...
                const page1 = yield apiUtils_1.getApi(session1.id, 'items/root/delta');
                expect(page1.items.map(i => i.item_name).sort()).toEqual(names);
                cursor1 = page1.cursor;
                const page2 = yield apiUtils_1.getApi(session2.id, 'items/root/delta');
            ...
            // --------------------------------------------------------------------
            const noteItem = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            const note = yield testUtils_1.models().item().loadAsJoplinItem(noteItem.id);
            yield time_1.msleep(1);
            yield testUtils_1.updateItem(session1.id, 'root:/00000000000000000000000000000001.md:', testUtils_1.makeNoteSerializedBody(Object.assign(Object.assign({}, note), { title: 'modified by user 1' })));
            {
                const page1 = yield apiUtils_1.getApi(session1.id, 'items/root/delta', { query: { cursor: cursor1 } });
                ...
                cursor1 = page1.cursor;
                const page2 = yield apiUtils_1.getApi(session2.id, 'items/root/delta', { query: { cursor: cursor2 } });
            ...
            // --------------------------------------------------------------------
            yield time_1.msleep(1);
            yield testUtils_1.updateItem(session2.id, 'root:/00000000000000000000000000000001.md:', testUtils_1.makeNoteSerializedBody(Object.assign(Object.assign({}, note), { title: 'modified by user 2' })));
            {
                const page1 = yield apiUtils_1.getApi(session1.id, 'items/root/delta', { query: { cursor: cursor1 } });
                expect(page1.items.length).toBe(1);
                expect(page1.items[0].item_name).toBe('00000000000000000000000000000001.md');
    ...
    test('should get delta changes - user 2 sync, user 1 share and sync, user 2 sync', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            yield testUtils_1.createItemTree(user2.id, '', {
                '200000000000000000000000000000F2': {},
            });
            let latestChanges2 = yield testUtils_1.models().change().delta(user2.id);
            ...
            latestChanges2 = yield testUtils_1.models().change().delta(user2.id, { cursor: cursor2 });
    ...
    test('should get delta changes - user 1 and 2 are in sync, user 2 adds a note to shared folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { share } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', {
                '000000000000000000000000000000F1': {
                    '00000000000000000000000000000001': null,
                },
            });
            let latestChanges1 = yield testUtils_1.models().change().delta(user1.id);
            const cursor1 = latestChanges1.cursor;
            const folderItem1 = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F1');
            yield testUtils_1.createNote(session2.id, {
                id: '00000000000000000000000000000002',
                title: 'from user 2',
            ...
            yield testUtils_1.models().share().updateSharedItems3();
            latestChanges1 = yield testUtils_1.models().change().delta(user1.id, { cursor: cursor1 });
    ...
    test('should get delta changes - user 1 and 2 are in sync, user 2 moves a note out of shared folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let latestChanges1 = yield testUtils_1.models().change().delta(user1.id);
            const cursor1 = latestChanges1.cursor;
            const folderItem2 = yield testUtils_1.createFolder(session2.id, { id: '000000000000000000000000000000F2', title: 'folder2' });
            const noteItem = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            const note = yield testUtils_1.models().item().loadAsJoplinItem(noteItem.id);
            yield testUtils_1.updateNote(session2.id, Object.assign(Object.assign({}, note), { parent_id: folderItem2.jop_id, share_id: '' }));
            yield testUtils_1.models().share().updateSharedItems3();
            latestChanges1 = yield testUtils_1.models().change().delta(user1.id, { cursor: cursor1 });
            expect(latestChanges1.items.length).toBe(1);
    ...
    test('should get delta changes - user 1 and 2 are in sync, user 1 deletes invitation', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const resourceItem1 = yield testUtils_1.createResource(session1.id, { id: '000000000000000000000000000000E1' }, 'testing1');
            const { shareUser, share } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
            ...
            const resourceBlob = yield testUtils_1.models().item().loadByName(user1.id, joplinUtils_1.resourceBlobPath(resourceItem1.jop_id));
            yield testUtils_1.models().item().saveForUser(user1.id, {
                id: resourceBlob.id,
                jop_share_id: share.id,
            });
            yield testUtils_1.models().share().updateSharedItems3();
            expect((yield testUtils_1.models().userItem().byUserId(user2.id)).length).toBe(4);
            yield apiUtils_1.deleteApi(session1.id, `share_users/${shareUser.id}`);
            expect((yield testUtils_1.models().userItem().byUserId(user2.id)).length).toBe(0);
        });
    });
    ...
    test('should unshare from a non-owner user who has deleted the root folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { item } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
            ...
            yield testUtils_1.models().share().updateSharedItems3();
            expect((yield testUtils_1.models().userItem().byUserId(user2.id)).length).toBe(2);
            yield apiUtils_1.deleteApi(session2.id, `items/root:/${item.name}:`);
            expect((yield testUtils_1.models().userItem().byUserId(user1.id)).length).toBe(2);
            expect((yield testUtils_1.models().userItem().byUserId(user2.id)).length).toBe(0);
        });
    });
    test('should unshare a folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { item, share } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
            ...
            yield apiUtils_1.deleteApi(session1.id, `shares/${share.id}`);
            yield testUtils_1.models().item().saveForUser(user1.id, { id: item.id, jop_share_id: '' });
            yield testUtils_1.models().item().saveForUser(user1.id, { id: noteItem.id, jop_share_id: '' });
            yield testUtils_1.models().share().updateSharedItems3();
            expect((yield testUtils_1.models().userItem().byUserId(user2.id)).length).toBe(0);
        });
    });
    ...
    test('should check permissions - cannot share a folder with yourself', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            yield testUtils_1.createItemTree(user1.id, '', { '000000000000000000000000000000F1': {} });
            const share = yield apiUtils_1.postApi(session1.id, 'shares', { folder_id: '000000000000000000000000000000F1' });
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.postApi(session1.id, `shares/${share.id}/users`, { email: user1.email }); }), errors_1.ErrorForbidden.httpCode);
        });
    });
    test('should check permissions - cannot share a folder twice with a user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield testUtils_1.createItemTree(user1.id, '', { '000000000000000000000000000000F1': {} });
            const share = yield apiUtils_1.postApi(session1.id, 'shares', { folder_id: '000000000000000000000000000000F1' });
            yield apiUtils_1.postApi(session1.id, `shares/${share.id}/users`, { email: user2.email });
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.postApi(session1.id, `shares/${share.id}/users`, { email: user2.email }); }), errors_1.ErrorForbidden.httpCode);
        });
    });
    test('should check permissions - cannot share a non-root folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            yield testUtils_1.createItemTree(user1.id, '', {
                '000000000000000000000000000000F1': {
                    '000000000000000000000000000000F2': {},
                },
            });
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.postApi(session1.id, 'shares', { folder_id: '000000000000000000000000000000F2' }); }), errors_1.ErrorForbidden.httpCode);
        });
    });
    test('should check permissions - cannot share if share feature not enabled', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { session: session2 } = yield testUtils_1.createUserAndSession(2);
            yield testUtils_1.models().user().save({ id: user1.id, can_share_folder: 0 });
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () {
                return shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                    {
                        id: '000000000000000000000000000000F1',
                        children: [],
                    },
    ...
    test('should check permissions - cannot share if share feature not enabled for recipient', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            yield testUtils_1.models().user().save({ id: user2.id, can_share_folder: 0 });
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () {
                return shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', [
                    {
                        id: '000000000000000000000000000000F1',
    ...
    test('should check permissions - by default sharing by note is always possible', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const noteItem = yield testUtils_1.createNote(session1.id, {
                title: 'Testing title',
                body: 'Testing body',
            });
            const share = yield apiUtils_1.postApi(session1.id, 'shares', {
    ...
    test('should allow sharing, unsharing and sharing again', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { session: session1 } = yield testUtils_1.createUserAndSession(1);
            const { user: user2, session: session2 } = yield testUtils_1.createUserAndSession(2);
            const { shareUser: shareUserA, share } = yield shareApiUtils_1.shareFolderWithUser(session1.id, session2.id, '000000000000000000000000000000F1', {
                '000000000000000000000000000000F1': {
                    '00000000000000000000000000000001': null,
                },
            });
            yield testUtils_1.models().share().updateSharedItems3();
            yield apiUtils_1.deleteApi(session1.id, `share_users/${shareUserA.id}`);
            yield testUtils_1.models().share().updateSharedItems3();
            yield shareApiUtils_1.inviteUserToShare(share, session1.id, user2.email, true);
            yield testUtils_1.models().share().updateSharedItems3();
            const page = yield apiUtils_1.getApi(session2.id, 'items/root/delta', { query: { cursor: '' } });
