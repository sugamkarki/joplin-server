...
router.post('api/debug', (_path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    if (config_1.default().env !== types_1.Env.Dev)
        throw new errors_1.ErrorForbidden();
    const query = (yield requestUtils_1.bodyFields(ctx.req));
    ...
    if (query.action === 'createTestUsers') {
        ...
        yield debugTools_1.createTestUsers(ctx.joplin.db, config_1.default(), options);
    }
    if (query.action === 'createUserDeletions') {
        yield debugTools_1.createUserDeletions(ctx.joplin.db, config_1.default());
    }
    if (query.action === 'clearDatabase') {
        yield debugTools_1.clearDatabase(ctx.joplin.db);
    }
