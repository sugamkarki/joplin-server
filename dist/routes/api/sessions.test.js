...
describe('api/sessions', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('api/sessions');
    }));
    ...
    test('should login user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user, password } = yield testUtils_1.createUserAndSession(1, false);
            const context = yield postSession(user.email, password);
            expect(context.response.status).toBe(200);
            expect(!!context.response.body.id).toBe(true);
            const session = yield testUtils_1.models().session().load(context.response.body.id);
            expect(session.user_id).toBe(user.id);
        });
    });
    test('should not login user with wrong password', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user } = yield testUtils_1.createUserAndSession(1, false);
            {
