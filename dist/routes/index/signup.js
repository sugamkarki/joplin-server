...
function makeView(error = null) {
    const view = defaultView_1.default('signup', 'Sign Up');
    view.content = {
        error,
        postUrl: routeUtils_1.makeUrl(routeUtils_1.UrlType.Signup),
        loginUrl: routeUtils_1.makeUrl(routeUtils_1.UrlType.Login),
    };
    return view;
}
...
router.post('signup', (_path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    if (!config_1.default().signupEnabled)
        throw new errors_1.ErrorForbidden('Signup is not enabled');
    try {
        const formUser = yield requestUtils_1.bodyFields(ctx.req);
        const password = users_1.checkRepeatPassword(formUser, true);
