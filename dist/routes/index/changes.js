...
router.get('changes', (_path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const pagination = table_1.makeTablePagination(ctx.query, 'updated_time', pagination_1.PaginationOrderDir.DESC);
            ...
            const row = [
                {
                    ...
                    stretch: true,
                    url: config_1.showItemUrls(config_1.default()) ? (items.find(i => i.id === change.item_id) ? ctx.joplin.models.item().itemContentUrl(change.item_id) : '') : null,
                },
                {
                    value: types_2.changeTypeToString(change.type),
                },
                {
                    value: time_1.formatDateTime(change.updated_time),
                },
            ];
            return row;
        }),
    };
    const view = defaultView_1.default('changes', 'Log');
    view.content.changeTable = table_1.makeTableView(table),
