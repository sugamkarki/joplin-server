...
function postUser(sessionId, email, password = null, props = null) {
    return __awaiter(this, void 0, void 0, function* () {
        password = password === null ? uuidgen_1.default() : password;
        ...
        testUtils_1.checkContextError(context);
...
describe('index/users', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('index_users');
    }));
    ...
    test('new user should be able to login', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const password = uuidgen_1.default();
            yield postUser(session.id, 'test@example.com', password);
            const loggedInUser = yield testUtils_1.models().user().login('test@example.com', password);
    ...
    test('should change user properties', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user, session } = yield testUtils_1.createUserAndSession(1, false);
            const userModel = testUtils_1.models().user();
    ...
    test('should get a user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user, session } = yield testUtils_1.createUserAndSession();
            const userHtml = yield getUserHtml(session.id, user.id);
            const doc = testUtils_1.parseHtml(userHtml);
    ...
    test('should allow user to set a password for new accounts', function () {
        return __awaiter(this, void 0, void 0, function* () {
            let user1 = yield testUtils_1.models().user().save({
                email: 'user1@localhost',
                must_set_password: 1,
                email_confirmed: 0,
                password: uuidgen_1.default(),
            });
            const { user: user2 } = yield testUtils_1.createUserAndSession(2);
            const email = (yield testUtils_1.models().email().all()).find(e => e.recipient_id === user1.id);
            ...
            yield apiUtils_1.execRequest('', 'GET', path, null, { query: { token } });
            // As soon as the confirmation page is opened, we know the email is valid
            user1 = yield testUtils_1.models().user().load(user1.id);
            expect(user1.email_confirmed).toBe(1);
            // Check that the token is valid
            expect(yield testUtils_1.models().token().isValid(user1.id, token)).toBe(true);
            // Check that we can't set the password without the token
            {
                const newPassword = uuidgen_1.default();
                const context = yield apiUtils_1.execRequestC('', 'POST', path, {
                    password: newPassword,
                    password2: newPassword,
                });
                const sessionId = cookies_1.cookieGet(context, 'sessionId');
                expect(sessionId).toBeFalsy();
            }
            // Check that we can't set the password with someone else's token
            {
                const newPassword = uuidgen_1.default();
                const token2 = (yield testUtils_1.models().token().allByUserId(user2.id))[0].value;
                const context = yield apiUtils_1.execRequestC('', 'POST', path, {
                    password: newPassword,
                    password2: newPassword,
                    token: token2,
            ...
            const sessionId = cookies_1.cookieGet(context, 'sessionId');
            const session = yield testUtils_1.models().session().load(sessionId);
            expect(session.user_id).toBe(user1.id);
            // Check that the password has been set
            const loggedInUser = yield testUtils_1.models().user().login(user1.email, newPassword);
            expect(loggedInUser.id).toBe(user1.id);
            // Check that the email has been verified
            expect(user1.email_confirmed).toBe(1);
            // Check that the token has been cleared
            expect(yield testUtils_1.models().token().isValid(user1.id, token)).toBe(false);
            // Check that a notification has been created
            const notification = (yield testUtils_1.models().notification().all())[0];
    ...
    test('should not confirm email if not requested', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const path = matches[1];
            const token = matches[3];
            yield apiUtils_1.execRequest('', 'GET', path, null, { query: { token, confirm_email: '0' } });
    ...
    test('should allow user to verify their email', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // const path = matches[1];
            // const token = matches[3];
            const context = yield apiUtils_1.execRequestC('', 'GET', path, null, { query: { token } });
            user1 = yield testUtils_1.models().user().load(user1.id);
            // Check that the user has been logged in
            const sessionId = cookies_1.cookieGet(context, 'sessionId');
            expect(sessionId).toBeFalsy();
    ...
    test('should allow changing an email', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            expect((yield testUtils_1.models().user().load(user.id)).email).toBe('user1@localhost');
            // Grab the confirmation URL
            const email = (yield testUtils_1.models().email().all()).find(e => e.recipient_id === user.id);
            const [, path, , token] = email.body.match(/\/(users\/.*)(\?token=)(.{32})/);
            yield apiUtils_1.execRequest('', 'GET', path, null, { query: { token } });
            // Now that it's confirmed, it should have been changed
            expect((yield testUtils_1.models().user().load(user.id)).email).toBe('changed@example.com');
            const keys = yield testUtils_1.models().keyValue().all();
            expect(keys.length).toBe(1);
            expect(keys[0].value).toBe('user1@localhost'); // The old email has been saved
            yield testUtils_1.expectThrow(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.execRequest('', 'GET', path, null, { query: { token } }); }));
        });
    });
    test('should not change non-whitelisted properties', () => __awaiter(this, void 0, void 0, function* () {
        const { user: user1, session: session1 } = yield testUtils_1.createUserAndSession(2, false);
        ...
        const reloadedUser1 = yield testUtils_1.models().user().load(user1.id);
    ...
    test('should apply ACL', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: admin } = yield testUtils_1.createUserAndSession(1, true);
            const { session: session1 } = yield testUtils_1.createUserAndSession(2, false);
            // non-admin cannot list users
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.execRequest(session1.id, 'GET', 'admin/users'); }), errors_1.ErrorForbidden.httpCode);
            // non-admin user cannot view another user
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return apiUtils_1.execRequest(session1.id, 'GET', `users/${admin.id}`); }), errors_1.ErrorForbidden.httpCode);
            // non-admin user cannot create a new user
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return postUser(session1.id, 'cantdothat@example.com'); }), errors_1.ErrorForbidden.httpCode);
            // non-admin user cannot update another user
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return patchUser(session1.id, { id: admin.id, email: 'cantdothateither@example.com' }); }), errors_1.ErrorForbidden.httpCode);
        });
    });
});
