...
function stripeEvent(stripe, req) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!stripe_1.stripeConfig().webhookSecret)
            throw new Error('webhookSecret is required');
        const body = yield getRawBody(req);
        return stripe.webhooks.constructEvent(body, req.headers['stripe-signature'], stripe_1.stripeConfig().webhookSecret);
    });
}
    ...
    if (existingUser) {
        ...
        if (!sub) {
            ...
            if (sub.stripe_subscription_id === stripeSubscriptionId) {
                ...
                yield stripe_1.cancelSubscription(stripe, stripeSubscriptionId);
            }
        }
    }
    else {
...
exports.postHandlers = {
    createCheckoutSession: (stripe, __path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const checkoutSession = {
            ...
            success_url: `${config_1.default().baseUrl}/stripe/success?session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `${config_1.default().baseUrl}/stripe/cancel`,
        };
        if (fields.coupon) {
        ...
        if (fields.email) {
            ...
            if (existingUser && (yield stripe_1.isBetaUser(ctx.joplin.models, existingUser.id))) {
                checkoutSession.subscription_data.trial_period_days = stripe_1.betaUserTrialPeriodDays(existingUser.created_time);
            }
        }
        ...
        const hooks = {
            ...
            'customer.subscription.created': () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                try {
                    ...
                    accountType = stripe_1.priceIdToAccountType(priceId);
                }
                catch (error) {
                    logger.error('Could not determine account type from price ID - defaulting to "Basic"', error);
                }
                yield exports.handleSubscriptionCreated(stripe, models, customer.name, customer.email, accountType, stripeUserId, stripeSubscriptionId);
            }),
            ...
            'customer.subscription.updated': () => __awaiter(void 0, void 0, void 0, function* () {
                ...
                const newAccountType = stripe_1.priceIdToAccountType(stripeSub.items.data[0].price.id);
...
const getHandlers = {
    success: (stripe, _path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            const redirectUrl = encodeURI(urlUtils_1.confirmUrl(user.id, validationToken, false));
            return routeUtils_1.redirect(ctx, redirectUrl);
        }
        catch (error) {
            ...
            return `
				<p>Thank you for signing up for ${config_1.default().appName}! You should receive an email shortly with instructions on how to connect to your account.</p>
    ...
    checkoutTest: (_stripe, _path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
        if (config_1.default().env === types_1.Env.Prod)
            throw new errors_1.ErrorForbidden();
        const basicPrice = joplinCloud_1.findPrice(stripe_1.stripeConfig().prices, { accountType: 1, period: joplinCloud_1.PricePeriod.Monthly });
        const proPrice = joplinCloud_1.findPrice(stripe_1.stripeConfig().prices, { accountType: 2, period: joplinCloud_1.PricePeriod.Monthly });
        ...
        return `
			<head>
				...
				<script>
					var stripe = Stripe(${JSON.stringify(stripe_1.stripeConfig().publishableKey)});
...
router.post('stripe/:id', (path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return exports.postHandlers[path.id](stripe_1.initStripe(), path, ctx);
}));
router.get('stripe/:id', (path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    if (!getHandlers[path.id])
        throw new errors_1.ErrorNotFound(`No such action: ${path.id}`);
    return getHandlers[path.id](stripe_1.initStripe(), path, ctx);
}));
