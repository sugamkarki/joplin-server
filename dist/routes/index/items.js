...
router.get('items', (_path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const pagination = table_1.makeTablePagination(ctx.query, 'name', pagination_1.PaginationOrderDir.ASC);
            ...
            const row = [
                {
                    value: item.name,
                    stretch: true,
                    url: config_1.showItemUrls(config_1.default()) ? `${config_1.default().userContentBaseUrl}/items/${item.id}/content` : null,
                },
                {
                    value: bytes_1.formatBytes(item.content_size),
                },
                {
                    value: item.mime_type || 'binary',
                },
                {
                    value: time_1.formatDateTime(item.updated_time),
                },
            ];
            return row;
        }),
    };
    const view = defaultView_1.default('items', 'Items');
    view.content.itemTable = table_1.makeTableView(table),
        view.content.postUrl = `${config_1.default().baseUrl}/items`;
...
router.get('items/:id/content', (path, ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return routeUtils_1.respondWithItemContent(ctx.response, item, item.content);
