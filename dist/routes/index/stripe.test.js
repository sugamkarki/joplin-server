...
function simulateWebhook(ctx, type, object, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        options = Object.assign({ stripe: mockStripe({ userEmail: options.userEmail }), eventId: uuidgen_1.default() }, options);
...
function createUserViaSubscription(ctx, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        options = Object.assign({ subscriptionId: `sub_${uuidgen_1.default()}`, customerId: `cus_${uuidgen_1.default()}` }, options);
        const stripeSessionId = 'sess_123';
        const stripePrice = joplinCloud_1.findPrice(stripe_1.stripeConfig().prices, { accountType: 2, period: joplinCloud_1.PricePeriod.Monthly });
        yield testUtils_1.models().keyValue().setValue(`stripeSessionToPriceId::${stripeSessionId}`, stripePrice.id);
...
describe('index/stripe', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('index/stripe');
    ...
    test('should not process the same event twice', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const v = yield testUtils_1.models().keyValue().value('stripeEventDone::evt_1');
            expect(v).toBe(1);
            // This event should simply be skipped
            yield testUtils_1.expectNotThrow(() => __awaiter(this, void 0, void 0, function* () { return createUserViaSubscription(ctx, { userEmail: 'toto@example.com', eventId: 'evt_1' }); }));
        });
    });
    test('should check if it is a beta user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const user1 = yield testUtils_1.models().user().save({ email: 'toto@example.com', password: uuidgen_1.default() });
            const user2 = yield testUtils_1.models().user().save({ email: 'tutu@example.com', password: uuidgen_1.default() });
            yield testUtils_1.models().user().save({ id: user2.id, created_time: 1624441295775 });
            expect(yield stripe_1.isBetaUser(testUtils_1.models(), user1.id)).toBe(false);
            expect(yield stripe_1.isBetaUser(testUtils_1.models(), user2.id)).toBe(true);
            ...
            expect(yield stripe_1.isBetaUser(testUtils_1.models(), user2.id)).toBe(false);
        });
    });
    test('should find out beta user trial end date', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const fromDateTime = 1627901594842; // Mon Aug 02 2021 10:53:14 GMT+0000
            expect(stripe_1.betaUserTrialPeriodDays(1624441295775, fromDateTime)).toBe(50); // Wed Jun 23 2021 09:41:35 GMT+0000
            expect(stripe_1.betaUserTrialPeriodDays(1614682158000, fromDateTime)).toBe(7); // Tue Mar 02 2021 10:49:18 GMT+0000
        });
    });
    test('should setup subscription for an existing user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // for instance.
            const ctx = yield testUtils_1.koaAppContext();
            const user = yield testUtils_1.models().user().save({ email: 'toto@example.com', password: uuidgen_1.default() });
            expect(yield testUtils_1.models().subscription().byUserId(user.id)).toBeFalsy();
            yield createUserViaSubscription(ctx, { userEmail: 'toto@example.com', subscriptionId: 'sub_123' });
            const sub = yield testUtils_1.models().subscription().byUserId(user.id);
    ...
    test('should cancel duplicate subscriptions', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            expect((yield testUtils_1.models().user().all()).length).toBe(1);
            const user = (yield testUtils_1.models().user().all())[0];
            const subBefore = yield testUtils_1.models().subscription().byUserId(user.id);
            expect(stripe.subscriptions.del).toHaveBeenCalledTimes(0);
            yield createUserViaSubscription(ctx, { userEmail: 'toto@example.com', stripe });
            expect((yield testUtils_1.models().user().all()).length).toBe(1);
            const subAfter = yield testUtils_1.models().subscription().byUserId(user.id);
    ...
    test('should disable an account if the sub is cancelled', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const user = (yield testUtils_1.models().user().all())[0];
            const sub = (yield testUtils_1.models().subscription().all())[0];
    ...
    test('should re-enable account if successful payment is made', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ctx = yield testUtils_1.koaAppContext();
            yield createUserViaSubscription(ctx, { userEmail: 'toto@example.com', stripe, subscriptionId: 'sub_init' });
            let user = (yield testUtils_1.models().user().all())[0];
            yield testUtils_1.models().user().save({
                id: user.id,
                enabled: 0,
                can_upload: 0,
            });
            yield testUtils_1.models().userFlag().add(user.id, types_1.UserFlagType.FailedPaymentFinal);
            yield simulateWebhook(ctx, 'invoice.paid', { subscription: 'sub_init' });
            user = yield testUtils_1.models().user().load(user.id);
            expect(user.enabled).toBe(1);
    ...
    test('should attach new sub to existing user', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const stripePrice = joplinCloud_1.findPrice(stripe_1.stripeConfig().prices, { accountType: 1, period: joplinCloud_1.PricePeriod.Monthly });
            yield simulateWebhook(ctx, 'customer.subscription.created', {
                id: 'sub_new',
                customer: 'cus_toto',
