...
const urlMatchesSchema = (url, schema) => {
    url = urlUtils_1.stripOffQueryParameters(url);
...
function disabledAccountCheck(route, user) {
    ...
    if (route.subPath.schema.startsWith('api/'))
        throw new errors_1.ErrorForbidden(`This account is disabled. Please login to ${config_1.default().baseUrl} for more information.`);
}
function execRequest(routes, ctx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (ctx.URL && !isValidOrigin(ctx.URL.origin, config_1.baseUrl(endPoint.type), endPoint.type))
        ...
        if (!isPublicRoute && !ctx.joplin.owner) {
            if (requestUtils_1.contextSessionId(ctx, false)) {
        ...
        yield csrf_1.csrfCheck(ctx, isPublicRoute);
...
function makeUrl(urlType) {
    if (config_1.default().isJoplinCloud && urlType === UrlType.Signup) {
        return `${config_1.default().joplinAppBaseUrl}/plans`;
    }
    else {
        return `${config_1.baseUrl(types_2.RouteType.Web)}/${urlType}`;
    }
}
