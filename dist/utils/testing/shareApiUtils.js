...
function createFolderShare(sessionId, folderId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return apiUtils_1.postApi(sessionId, 'shares', {
...
function createItemTree3(sessionId, userId, parentFolderId, shareId, tree) {
    return __awaiter(this, void 0, void 0, function* () {
        const user = yield testUtils_1.models().user().load(userId);
        for (const jopItem of tree) {
            const isFolder = !!jopItem.children;
            const serializedBody = isFolder ?
                testUtils_1.makeFolderSerializedBody(Object.assign(Object.assign({}, jopItem), { parent_id: parentFolderId, share_id: shareId })) :
                testUtils_1.makeNoteSerializedBody(Object.assign(Object.assign({}, jopItem), { parent_id: parentFolderId, share_id: shareId }));
            if (!isFolder) {
                const resourceIds = joplinUtils_1.linkedResourceIds(jopItem.body || '');
                for (const resourceId of resourceIds) {
                    yield testUtils_1.createResource(sessionId, { id: resourceId, share_id: shareId }, `testing-${resourceId}`);
                }
            }
            const result = yield testUtils_1.models().item().saveFromRawContent(user, [{ name: `${jopItem.id}.md`, body: Buffer.from(serializedBody) }]);
...
function inviteUserToShare(share, sharerSessionId, recipientEmail, acceptShare = true) {
    return __awaiter(this, void 0, void 0, function* () {
        let shareUser = yield apiUtils_1.postApi(sharerSessionId, `shares/${share.id}/users`, {
            email: recipientEmail,
        });
        shareUser = yield testUtils_1.models().shareUser().load(shareUser.id);
        if (acceptShare) {
            const session = yield testUtils_1.models().session().createUserSession(shareUser.user_id);
            yield apiUtils_1.patchApi(session.id, `share_users/${shareUser.id}`, { status: types_1.ShareUserStatus.Accepted });
        }
        return shareUser;
    });
}
...
function shareFolderWithUser(sharerSessionId, shareeSessionId, sharedFolderId, itemTree, acceptShare = true) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const sharee = yield testUtils_1.models().session().sessionUser(shareeSessionId);
        const sharer = yield testUtils_1.models().session().sessionUser(sharerSessionId);
        const rootFolderItem = yield testUtils_1.createFolder(sharerSessionId, {
            id: sharedFolderId,
            title: 'folder 1',
        });
        const share = yield apiUtils_1.postApi(sharerSessionId, 'shares', {
            type: types_1.ShareType.Folder,
            folder_id: rootFolderItem.jop_id,
        });
        const rootFolder = yield testUtils_1.models().item().loadAsJoplinItem(rootFolderItem.id);
        yield testUtils_1.updateFolder(sharerSessionId, Object.assign(Object.assign({}, rootFolder), { share_id: share.id }));
...
function shareWithUserAndAccept(sharerSessionId, shareeSessionId, sharee, shareType = types_1.ShareType.Folder, item = null) {
    return __awaiter(this, void 0, void 0, function* () {
        item = item || (yield testUtils_1.createItem(sharerSessionId, 'root:/test.txt:', 'testing share'));
        let share = null;
        if ([types_1.ShareType.Folder, types_1.ShareType.Note].includes(shareType)) {
            share = yield apiUtils_1.postApi(sharerSessionId, 'shares', {
                type: shareType,
            ...
            const sharer = yield testUtils_1.models().session().sessionUser(sharerSessionId);
            share = yield testUtils_1.models().share().save({
                owner_id: sharer.id,
                type: shareType,
                item_id: item.id,
            });
...
function respondInvitation(recipientSessionId, shareUserId, status) {
    return __awaiter(this, void 0, void 0, function* () {
        yield apiUtils_1.patchApi(recipientSessionId, `share_users/${shareUserId}`, { status });
    });
}
