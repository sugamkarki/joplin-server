...
function stripeConfig() {
    return config_1.default().stripe;
}
...
function priceIdToAccountType(priceId) {
    const price = joplinCloud_1.findPrice(stripeConfig().prices, { priceId });
    return price.accountType;
}
exports.priceIdToAccountType = priceIdToAccountType;
function accountTypeToPriceId(accountType) {
    const price = joplinCloud_1.findPrice(stripeConfig().prices, { accountType, period: joplinCloud_1.PricePeriod.Monthly });
    return price.id;
}
...
function updateSubscriptionType(models, userId, newAccountType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const currentPrice = joplinCloud_1.findPrice(stripeConfig().prices, { priceId: stripePriceIdByStripeSub(stripeSub) });
        const upgradePrice = joplinCloud_1.findPrice(stripeConfig().prices, { accountType: newAccountType, period: currentPrice.period });
        const items = [];
...
function betaStartSubUrl(email, accountType) {
    return `${config_1.default().joplinAppBaseUrl}/plans/?email=${encodeURIComponent(email)}&account_type=${encodeURIComponent(accountType)}`;
}
