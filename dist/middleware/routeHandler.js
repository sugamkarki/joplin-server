...
function default_1(ctx) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const { response: responseObject, path } = yield routeUtils_1.execRequest(ctx.joplin.routes, ctx);
            if (responseObject instanceof routeUtils_1.Response) {
                ctx.response = responseObject.response;
            }
            else if (MustacheService_1.isView(responseObject)) {
                const impersonatorAdminSessionId = impersonate_1.getImpersonatorAdminSessionId(ctx);
                ...
                ctx.response.body = yield ctx.joplin.services.mustache.renderView(view, {
                    ...
                    csrfTag: impersonatorAdminSessionId ? yield csrf_1.createCsrfTag(ctx, false) : null,
                });
            }
            else {
            ...
            if (error.httpCode >= 400 && error.httpCode < 500) {
                ...
                const line = [
                    ...
                    owner ? owner.id : requestUtils_1.userIp(ctx),
                    error.message,
                ];
                if (error.details)
                ...
                ctx.joplin.appLogger().error(requestUtils_1.userIp(ctx), error);
            }
            ...
            if (error.code === 'invalidOrigin') {
                ...
                const view = {
                    ...
                    content: {
                        error,
                        stack: config_1.default().showErrorStackTraces ? error.stack : '',
                        owner: ctx.joplin.owner,
                    },
                    title: 'Error',
                };
