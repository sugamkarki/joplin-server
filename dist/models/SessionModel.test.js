...
describe('SessionModel', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('SessionModel');
    }));
    ...
    test('should delete expired sessions', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const lastSession = yield testUtils_1.models().session().authenticate(user.email, password);
            expect(yield testUtils_1.models().session().count()).toBe(3);
            yield testUtils_1.models().session().deleteExpiredSessions();
            expect(yield testUtils_1.models().session().count()).toBe(1);
            expect((yield testUtils_1.models().session().all())[0].id).toBe(lastSession.id);
            yield testUtils_1.models().session().authenticate(user.email, password);
            yield testUtils_1.models().session().deleteExpiredSessions();
