...
function default_1(config, db, options = null) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (typeof config === 'number') {
            storageId = config;
            const models = factory_1.default(db, config_1.default());
            const storage = yield models.storage().byId(storageId);
            if (!storage)
                throw new Error(`No such storage ID: ${storageId}`);
            config = parseStorageConnectionString_1.default(storage.connection_string);
        }
        else {
            if (options.assignDriverId) {
                const models = factory_1.default(db, config_1.default());
                const connectionString = serializeStorageConfig_1.default(config);
        ...
        if (config.type === types_1.StorageDriverType.Database) {
            return new StorageDriverDatabase_1.default(storageId, Object.assign(Object.assign({}, config), { dbClientType: db_1.clientType(db) }));
        }
