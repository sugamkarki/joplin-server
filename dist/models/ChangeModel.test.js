...
describe('ChangeModel', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('ChangeModel');
    }));
    ...
    test('should track changes - create only', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { session, user } = yield testUtils_1.createUserAndSession(1, true);
            const changeModel = testUtils_1.models().change();
            const item1 = yield testUtils_1.createFolder(session.id, { title: 'folder' });
            {
    ...
    test('should track changes - create, then update', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user } = yield testUtils_1.createUserAndSession(1, true);
            const itemModel = testUtils_1.models().item();
            const changeModel = testUtils_1.models().change();
            yield time_1.msleep(1);
            const item1 = yield testUtils_1.models().item().makeTestItem(user.id, 1); // [1] CREATE 1
            yield time_1.msleep(1);
            ...
            yield time_1.msleep(1);
            const item2 = yield testUtils_1.models().item().makeTestItem(user.id, 2); // [4] CREATE 2
            yield time_1.msleep(1);
            ...
            yield time_1.msleep(1);
            const item3 = yield testUtils_1.models().item().makeTestItem(user.id, 3); // [8] CREATE 3
    ...
    test('should throw an error if cursor is invalid', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let i = 1;
            yield time_1.msleep(1);
            const item1 = yield testUtils_1.models().item().makeTestItem(user.id, 1); // CREATE 1
            yield time_1.msleep(1);
            yield itemModel.saveForUser(user.id, { id: item1.id, name: `test_mod${i++}`, content: Buffer.from('') }); // UPDATE 1
            yield testUtils_1.expectThrow(() => __awaiter(this, void 0, void 0, function* () { return changeModel.delta(user.id, { limit: 1, cursor: 'invalid' }); }), 'resyncRequired');
        });
    });
    test('should tell that there are more changes even when current page is empty', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield testUtils_1.createItemTree3(user1.id, '', '', itemsToCreate);
            yield testUtils_1.models().item().deleteAll(user1.id);
            expect((yield testUtils_1.models().change().all()).length).toBe(changeCount);
            ...
            const allFromIds1 = yield testUtils_1.models().change().allFromId('', changeCount / 2);
            expect(allFromIds1.items.length).toBe(0);
            expect(allFromIds1.has_more).toBe(true);
            const allFromIds2 = yield testUtils_1.models().change().allFromId(allFromIds1.cursor, changeCount / 2);
            expect(allFromIds2.items.length).toBe(5);
            expect(allFromIds2.has_more).toBe(true);
            const allFromIds3 = yield testUtils_1.models().change().allFromId(allFromIds2.cursor, changeCount / 2);
    ...
    test('should not fail when retrieving many changes', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { user } = yield testUtils_1.createUserAndSession(1, true);
            yield testUtils_1.models().item().makeTestItems(user.id, 1010);
            let changeCount = 0;
            yield testUtils_1.expectNotThrow(() => __awaiter(this, void 0, void 0, function* () {
                const changes = yield testUtils_1.models().change().allFromId('');
    ...
    test('should delete old changes', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const note1 = yield testUtils_1.createNote(session1.id, {});
            const t2 = new Date('2020-01-10').getTime();
            jest.setSystemTime(t2);
            const note2 = yield testUtils_1.createNote(session2.id, {});
            yield testUtils_1.updateNote(session1.id, { id: note1.jop_id });
            ...
            yield testUtils_1.updateNote(session2.id, { id: note2.jop_id });
            const t6 = new Date('2020-02-20').getTime();
            jest.setSystemTime(t6);
            yield testUtils_1.updateNote(session2.id, { id: note2.jop_id });
            expect(yield testUtils_1.models().change().count()).toBe(7);
            // Shouldn't do anything initially because it only deletes old changes.
            yield testUtils_1.models().change().compressOldChanges();
            expect(yield testUtils_1.models().change().count()).toBe(7);
            // 180 days after T4, it should delete all U1 updates events except for
            // the last one
            jest.setSystemTime(new Date(t4 + changeTtl).getTime());
            yield testUtils_1.models().change().compressOldChanges();
            expect(yield testUtils_1.models().change().count()).toBe(5);
            {
                const updateChange = (yield testUtils_1.models().change().all()).find(c => c.item_id === note1.id && c.type === types_1.ChangeType.Update);
            ...
            // been made later
            expect((yield testUtils_1.models().change().all()).filter(c => c.item_id === note2.id).length).toBe(3);
            ...
            yield testUtils_1.models().change().compressOldChanges();
            expect(yield testUtils_1.models().change().count()).toBe(4);
            {
                const updateChange = (yield testUtils_1.models().change().all()).find(c => c.item_id === note2.id && c.type === types_1.ChangeType.Update);
