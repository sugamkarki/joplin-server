...
describe('ItemModel', function () {
    beforeAll(() => __awaiter(this, void 0, void 0, function* () {
        yield testUtils_1.beforeAllDb('ItemModel');
    }));
    ...
    test('should find all items within a shared folder', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const resourceItem2 = yield testUtils_1.createResource(session1.id, { id: '000000000000000000000000000000E2' }, 'testing2');
            ...
            yield testUtils_1.createNote(session2.id, { id: '00000000000000000000000000000003', parent_id: '000000000000000000000000000000F1' });
            {
                const shareUserIds = yield testUtils_1.models().share().allShareUserIds(share);
                const children = yield testUtils_1.models().item().sharedFolderChildrenItems(shareUserIds, '000000000000000000000000000000F1');
                ...
                expect(children.filter(c => !c.jop_id).map(c => c.name).sort()).toEqual([
                    joplinUtils_1.resourceBlobPath('000000000000000000000000000000E1'),
                    joplinUtils_1.resourceBlobPath('000000000000000000000000000000E2'),
                ].sort());
            }
            {
                const children = yield testUtils_1.models().item().sharedFolderChildrenItems([user1.id], '000000000000000000000000000000F2');
    ...
    test('should count items', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1 } = yield testUtils_1.createUserAndSession(1, true);
            yield testUtils_1.createItemTree(user1.id, '', {
                '000000000000000000000000000000F1': {
                    '00000000000000000000000000000001': null,
                },
            });
            expect(yield testUtils_1.models().item().childrenCount(user1.id)).toBe(2);
        });
    });
    test('should calculate the total size', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield testUtils_1.createItemTree3(user1.id, '', '', [
                {
                    id: '000000000000000000000000000000F1',
                    children: [
                        {
            ...
            yield testUtils_1.createItemTree3(user2.id, '', '', [
                {
                    id: '000000000000000000000000000000F2',
                    children: [
                        {
            ...
            const folder1 = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F1');
            const folder2 = yield testUtils_1.models().item().loadByJopId(user2.id, '000000000000000000000000000000F2');
            const note1 = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            const note2 = yield testUtils_1.models().item().loadByJopId(user2.id, '00000000000000000000000000000002');
            const note3 = yield testUtils_1.models().item().loadByJopId(user2.id, '00000000000000000000000000000003');
            const totalSize1 = yield testUtils_1.models().item().calculateUserTotalSize(user1.id);
            const totalSize2 = yield testUtils_1.models().item().calculateUserTotalSize(user2.id);
            const totalSize3 = yield testUtils_1.models().item().calculateUserTotalSize(user3.id);
            ...
            yield testUtils_1.models().item().updateTotalSizes();
            expect((yield testUtils_1.models().user().load(user1.id)).total_item_size).toBe(totalSize1);
            expect((yield testUtils_1.models().user().load(user2.id)).total_item_size).toBe(totalSize2);
            expect((yield testUtils_1.models().user().load(user3.id)).total_item_size).toBe(totalSize3);
        });
    });
    test('should update total size when an item is deleted', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const note1 = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            yield testUtils_1.models().item().updateTotalSizes();
            expect((yield testUtils_1.models().user().load(user1.id)).total_item_size).toBe(folder1.content_size + note1.content_size);
            yield testUtils_1.models().item().delete(note1.id);
            yield testUtils_1.models().item().updateTotalSizes();
            expect((yield testUtils_1.models().user().load(user1.id)).total_item_size).toBe(folder1.content_size);
        });
    });
    test('should include shared items in total size calculation', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const folder1 = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F1');
            const folder2 = yield testUtils_1.models().item().loadByJopId(user1.id, '000000000000000000000000000000F2');
            const note1 = yield testUtils_1.models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
            const totalSize1 = yield testUtils_1.models().item().calculateUserTotalSize(user1.id);
            const totalSize2 = yield testUtils_1.models().item().calculateUserTotalSize(user2.id);
            const totalSize3 = yield testUtils_1.models().item().calculateUserTotalSize(user3.id);
            ...
            expect(totalSize3).toBe(expected3);
            yield testUtils_1.models().item().updateTotalSizes();
            expect((yield testUtils_1.models().user().load(user1.id)).total_item_size).toBe(expected1);
            expect((yield testUtils_1.models().user().load(user2.id)).total_item_size).toBe(expected2);
            expect((yield testUtils_1.models().user().load(user3.id)).total_item_size).toBe(expected3);
        });
    });
    test('should respect the hard item size limit', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const { user: user1 } = yield testUtils_1.createUserAndSession(1);
            let models = factory_1.default(testUtils_1.db(), config_1.default());
            let result = yield models.item().saveFromRawContent(user1, {
                body: Buffer.from('1234'),
                name: 'test1.txt',
            });
            const item = result['test1.txt'].item;
            config_1.default().itemSizeHardLimit = 3;
            models = factory_1.default(testUtils_1.db(), config_1.default());
            ...
            yield testUtils_1.expectHttpError(() => __awaiter(this, void 0, void 0, function* () { return models.item().loadWithContent(item.id); }), errors_1.ErrorPayloadTooLarge.httpCode);
            config_1.default().itemSizeHardLimit = 1000;
            models = factory_1.default(testUtils_1.db(), config_1.default());
            yield testUtils_1.expectNotThrow(() => __awaiter(this, void 0, void 0, function* () { return models.item().loadWithContent(item.id); }));
        });
    });
    const setupImportContentTest = () => __awaiter(this, void 0, void 0, function* () {
        const tempDir1 = yield testUtils_1.tempDir('storage1');
        const tempDir2 = yield testUtils_1.tempDir('storage2');
        ...
        const fromModels = factory_1.default(testUtils_1.db(), Object.assign(Object.assign({}, config_1.default()), { storageDriver: fromStorageConfig }));
        const toModels = factory_1.default(testUtils_1.db(), Object.assign(Object.assign({}, config_1.default()), { storageDriver: toStorageConfig }));
        const fromDriver = yield loadStorageDriver_1.default(fromStorageConfig, testUtils_1.db());
        const toDriver = yield loadStorageDriver_1.default(toStorageConfig, testUtils_1.db());
        return {
            fromStorageConfig,
            toStorageConfig,
            fromModels,
    ...
    test('should delete the database item content', function () {
        return __awaiter(this, void 0, void 0, function* () {
            if (db_1.isSqlite(testUtils_1.db())) {
                expect(1).toBe(1);
                return;
            }
            const { user: user1 } = yield testUtils_1.createUserAndSession(1);
            ...
            const folder1 = yield testUtils_1.models().item().loadByName(user1.id, '000000000000000000000000000000F1.md');
            const note1 = yield testUtils_1.models().item().loadByName(user1.id, '00000000000000000000000000000001.md');
            yield time_1.msleep(1);
            expect(yield testUtils_1.models().item().dbContent(folder1.id)).not.toEqual(Buffer.from(''));
            expect(yield testUtils_1.models().item().dbContent(note1.id)).not.toEqual(Buffer.from(''));
            yield testUtils_1.models().item().deleteDatabaseContentColumn({ batchSize: 1 });
            const folder1_v2 = yield testUtils_1.models().item().loadByName(user1.id, '000000000000000000000000000000F1.md');
            const note1_v2 = yield testUtils_1.models().item().loadByName(user1.id, '00000000000000000000000000000001.md');
            expect(folder1.updated_time).toBe(folder1_v2.updated_time);
            expect(note1.updated_time).toBe(note1_v2.updated_time);
            expect(yield testUtils_1.models().item().dbContent(folder1.id)).toEqual(Buffer.from(''));
            expect(yield testUtils_1.models().item().dbContent(note1.id)).toEqual(Buffer.from(''));
        });
    });
    test('should delete the database item content - maxProcessedItems handling', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield testUtils_1.models().item().deleteDatabaseContentColumn({ batchSize: 2, maxProcessedItems: 4 });
            const itemIds = (yield testUtils_1.models().item().all()).map(it => it.id);
            const contents = yield Promise.all([
                testUtils_1.models().item().dbContent(itemIds[0]),
                testUtils_1.models().item().dbContent(itemIds[1]),
                testUtils_1.models().item().dbContent(itemIds[2]),
                testUtils_1.models().item().dbContent(itemIds[3]),
                testUtils_1.models().item().dbContent(itemIds[4]),
            ]);
