...
class UserModel extends BaseModel_1.default {
    ...
    checkMaxItemSizeLimit(user, buffer, item, joplinItem) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const maxItemSize = user_1.getMaxItemSize(user);
            const maxSize = maxItemSize * (joplinUtils_1.itemIsEncrypted(item) ? 2.2 : 1);
            ...
            if (maxSize && itemSize > maxSize) {
                throw new errors_1.ErrorPayloadTooLarge(locale_1._('Cannot save %s "%s" because it is larger than the allowed limit (%s)', isNote ? locale_1._('note') : locale_1._('attachment'), itemTitle ? itemTitle : item.name, bytes_1.formatBytes(maxItemSize)));
            }
            ...
            if (!isWhiteListed) {
                ...
                const maxTotalItemSize = user_1.getMaxTotalItemSize(user) * 1.5;
                if (maxTotalItemSize && user.total_item_size + itemSize >= maxTotalItemSize) {
                    throw new errors_1.ErrorPayloadTooLarge(locale_1._('Cannot save %s "%s" because it would go over the total allowed size (%s) for this account', isNote ? locale_1._('note') : locale_1._('attachment'), itemTitle ? itemTitle : item.name, bytes_1.formatBytes(maxTotalItemSize)));
                }
            }
        });
    }
    ...
    sendAccountConfirmationEmail(user) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const url = encodeURI(urlUtils_1.confirmUrl(user.id, validationToken));
            yield this.models().email().push(Object.assign(Object.assign({}, accountConfirmationTemplate_1.default({ url })), this.userEmailDetails(user)));
        });
    }
    sendChangeEmailConfirmationEmail(recipientEmail, user) {
        return __awaiter(this, void 0, void 0, function* () {
            const validationToken = yield this.models().token().generate(user.id);
            const url = encodeURI(urlUtils_1.confirmUrl(user.id, validationToken));
            yield this.models().email().push(Object.assign(Object.assign(Object.assign({}, changeEmailConfirmationTemplate_1.default({ url })), this.userEmailDetails(user)), { recipient_email: recipientEmail }));
        });
    }
    sendChangeEmailNotificationEmail(recipientEmail, user) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.models().email().push(Object.assign(Object.assign(Object.assign({}, changeEmailNotificationTemplate_1.default()), this.userEmailDetails(user)), { recipient_email: recipientEmail }));
        });
    }
    sendResetPasswordEmail(email) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const validationToken = yield this.models().token().generate(user.id);
            const url = urlUtils_1.resetPasswordUrl(validationToken);
            yield this.models().email().push(Object.assign(Object.assign({}, resetPasswordTemplate_1.default({ url })), this.userEmailDetails(user)));
        });
    }
    resetPassword(token, fields) {
        return __awaiter(this, void 0, void 0, function* () {
            users_1.checkRepeatPassword(fields, true);
    ...
    handleBetaUserEmails() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!stripe_1.stripeConfig().enabled)
                return;
            ...
            for (const user of betaUsers) {
                if (!(yield stripe_1.isBetaUser(this.models(), user.id)))
                    continue;
                const remainingDays = stripe_1.betaUserTrialPeriodDays(user.created_time, 0, 0);
                for (const reminderInterval of reminderIntervals) {
                    if (remainingDays <= reminderInterval) {
                        ...
                        if (!(yield this.models().keyValue().value(sentKey))) {
                            yield this.models().email().push(Object.assign(Object.assign({}, endOfBetaTemplate_1.default({
                                expireDays: remainingDays,
                                startSubUrl: stripe_1.betaStartSubUrl(user.email, user.account_type),
            ...
            const subInfos = [
                {
                    ...
                    templateFn: () => paymentFailedUploadDisabledTemplate_1.default({ disabledInDays: Math.round(SubscriptionModel_1.failedPaymentFinalAccount / time_1.Day) }),
                },
                {
                    ...
                    templateFn: () => paymentFailedAccountDisabledTemplate_1.default(),
                },
            ];
            let users = [];
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.withTransaction(() => __awaiter(this, void 0, void 0, function* () {
                for (const user of users) {
                    const maxTotalItemSize = user_1.getMaxTotalItemSize(user);
                    const account = accountByType(user.account_type);
                    if (user.total_item_size > maxTotalItemSize * alertLimitMax) {
                        yield this.models().email().push(Object.assign(Object.assign(Object.assign({}, oversizedAccount2_1.default({
                        ...
                        yield this.models().email().push(Object.assign(Object.assign(Object.assign({}, oversizedAccount1_1.default({
