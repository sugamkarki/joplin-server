...
const senderInfo = (senderId) => {
    if (!senders_[senderId]) {
        if (senderId === types_1.EmailSender.NoReply) {
            senders_[senderId] = {
                name: config_1.default().mailer.noReplyName,
                email: config_1.default().mailer.noReplyEmail,
            };
        }
        else if (senderId === types_1.EmailSender.Support) {
            senders_[senderId] = {
                name: config_1.default().supportName,
                email: config_1.default().supportEmail,
            };
        }
        else {
