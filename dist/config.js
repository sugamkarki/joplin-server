...
function databaseConfigFromEnv(runningInDocker, env) {
    ...
    if (env.DB_CLIENT === 'pg') {
        return Object.assign(Object.assign({}, baseConfig), { client: types_1.DatabaseConfigClient.PostgreSQL, name: env.POSTGRES_DATABASE, user: env.POSTGRES_USER, password: env.POSTGRES_PASSWORD, port: env.POSTGRES_PORT, host: databaseHostFromEnv(runningInDocker, env) || 'localhost' });
    }
...
function baseUrlFromEnv(env, appPort) {
    if (env.APP_BASE_URL) {
        return path_utils_1.rtrimSlashes(env.APP_BASE_URL);
    }
    else {
...
function initConfig(envType, env, overrides = null) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const stripePublicConfig = joplinCloud_1.loadStripeConfig(envType === types_1.Env.BuildTypes ? types_1.Env.Dev : envType, `${rootDir}/stripeConfig.json`);
